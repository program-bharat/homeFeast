...
const Login = () => {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!form.email || !form.password) return notify.error('Email and password are required.');
        if (!/\S+@\S+\.\S+/.test(form.email)) return notify.error('Please enter a valid email address.');
        ...
        try {
            ...
            const destination = ROLE_REDIRECT[userData.role] ?? '/';
    ...
    return (
        <main className="flex min-h-screen overflow-hidden">
            <section className="hidden lg:flex lg:w-3/5 relative h-screen bg-[#3c2d27] overflow-hidden">
                <img
                    ref={imgRef}
                    src="https://plus.unsplash.com/premium_photo-1675252369719-dd52bc69c3df?w=600&auto=format&fit=crop&q=60"
                ...
                <div className="relative z-10 flex flex-col justify-end p-20 h-full w-full">
                    <div className="mb-4">
                        <span className="text-white block"
                            style={{ fontFamily: 'var(--font-heading)', fontSize: '48px', lineHeight: '56px', letterSpacing: '-0.02em', fontWeight: 700 }}>
                            HomeFeast
                        </span>
            ...
            <section className="w-full lg:w-2/5 flex flex-col justify-center items-center bg-[var(--color-background)] py-12 pt-20 md:px-20">
                <div className="w-full max-w-sm flex flex-col items-center">
                    <div className="lg:hidden mb-12 flex flex-col items-center">
                        <span className="text-[var(--color-primary)]"
                            style={{ fontFamily: 'var(--font-heading)', fontSize: '32px', lineHeight: '40px', letterSpacing: '-0.01em', fontWeight: 600 }}>
                            HomeFeast
                        </span>
                    </div>
                    <div className="w-full mb-10 text-center lg:text-left">
                        <h1 className="text-[var(--color-text)] mb-2"
                            style={{ fontFamily: 'var(--font-heading)', fontSize: '32px', lineHeight: '40px', letterSpacing: '-0.01em', fontWeight: 600 }}>
                            Welcome Back
                        </h1>
                    ...
                    <form onSubmit={handleSubmit} className="w-full space-y-6">
                        <div className="space-y-2">
                            <label htmlFor="email" className="block text-[var(--color-text-muted)]"
                                style={{ fontSize: '14px', lineHeight: '16px', letterSpacing: '0.01em', fontWeight: 600 }}>
                                Email Address
                            </label>
                        ...
                        <div className="space-y-2">
                            <div className="flex justify-between items-center">
                                <label htmlFor="password" className="block text-[var(--color-text-muted)]"
                                    style={{ fontSize: '14px', lineHeight: '16px', letterSpacing: '0.01em', fontWeight: 600 }}>
                                    Password
                                </label>
                            </div>
                        ...
                        <button type="submit" disabled={loading}
                            className="w-full py-4 bg-[var(--color-primary-dark)] text-white rounded-lg shadow-sm hover:bg-[var(--color-primary)] active:scale-[0.98] transition-all flex justify-center items-center gap-2 disabled:opacity-70 disabled:cursor-wait cursor-pointer"
                            style={{ fontSize: '14px', lineHeight: '16px', letterSpacing: '0.01em', fontWeight: 600 }}>
                    ...
                    <div className="mt-12 text-center">
                        <p className="text-[var(--color-text-muted)]" style={{ fontSize: '14px', lineHeight: '20px' }}>
                ...
                <footer className="mt-auto pt-10">
                    <nav className="flex gap-4 items-center justify-center">
                        <a href="#" className="text-[var(--color-outline)] hover:text-[var(--color-primary)] transition-colors"
                            style={{ fontSize: '12px', lineHeight: '16px', fontWeight: 600 }}>Privacy Policy</a>
                        <span className="w-1 h-1 rounded-full bg-[var(--color-border)]" />
                        <a href="#" className="text-[var(--color-outline)] hover:text-[var(--color-primary)] transition-colors"
                            style={{ fontSize: '12px', lineHeight: '16px', fontWeight: 600 }}>Terms of Service</a>
                    </nav>
                    <p className="text-center mt-4 text-[var(--color-outline)] opacity-60"
